...
void send_write(void *data) {
    MPI_Request r;
    // MPI_Status st;
    struct data_write *d_w = data;
    struct message *m = generate_message(DEF_NODE_USER, DEF_NODE_LEADER, DEF_NODE_LEADER,
    ...
    MPI_Isend(d_w->data, d_w->size, MPI_CHAR, m->id_t, 0, MPI_COMM_WORLD, &r);
}

void send_malloc(void *data) {
    MPI_Request r;
    // MPI_Status st;
    ...
    MPI_Irecv(buff, sizeof(struct message), MPI_CHAR, DEF_NODE_LEADER, 0, MPI_COMM_WORLD, &r2);
...
void send_read(void *data) {
    ...
    MPI_Irecv(buff, d_r->size * sizeof(char), MPI_CHAR, DEF_NODE_LEADER, 0, MPI_COMM_WORLD, &r2);
    while (0 != MPI_Wait(&r, &st)) {
        char *c_buff = buff;
        c_buff[d_r->size] = '\0';
        char *msg = "Read : ";
